...
import com.ConnectionProject.connection.libs.BufferedDataSink;
...
import com.ConnectionProject.connection.libs.http.filter.ChunkedOutputFilter;
...
public class HttpTransportMiddleware extends SimpleMiddleware {
    ...
    public boolean exchangeHeaders(final OnExchangeHeaderData data) {
        com.ConnectionProject.connection.libs.http.Protocol p = com.ConnectionProject.connection.libs.http.Protocol.get(data.protocol);
        if (p != null && p != com.ConnectionProject.connection.libs.http.Protocol.HTTP_1_0 && p != com.ConnectionProject.connection.libs.http.Protocol.HTTP_1_1)
        ...
        LineEmitter.StringCallback headerCallback = new LineEmitter.StringCallback() {
            com.ConnectionProject.connection.libs.http.Headers mRawHeaders = new Headers();
            String statusLine;
                ...
                try {
                    ...
                    if (statusLine == null) {
                        ...
                        if (!data.request.hasBody()) {
                            emitter = com.ConnectionProject.connection.libs.http.HttpUtil.EndEmitter.create(socket.getServer(), null);
                        }
                        else if (responseIsEmpty(data.response.code())) {
                            emitter = com.ConnectionProject.connection.libs.http.HttpUtil.EndEmitter.create(socket.getServer(), null);
                        }
                        else {
                            emitter = HttpUtil.getBodyDecoder(socket, com.ConnectionProject.connection.libs.http.Protocol.get(protocol), mRawHeaders, false);
                        }
    ...
    public void onRequestSent(OnRequestSentData data) {
        com.ConnectionProject.connection.libs.http.Protocol p = com.ConnectionProject.connection.libs.http.Protocol.get(data.protocol);
        if (p != null && p != com.ConnectionProject.connection.libs.http.Protocol.HTTP_1_0 && p != Protocol.HTTP_1_1)
            return;
