...
public class SimpleCancellable implements DependentCancellable {
    ...
    public boolean cancel() {
        com.example.connection.libs.future.Cancellable parent;
        synchronized (this) {
            if (complete)
                return false;
            if (cancelled)
    ...
    private com.example.connection.libs.future.Cancellable parent;
    @Override
    public boolean setParent(com.example.connection.libs.future.Cancellable parent) {
        synchronized (this) {
            if (isDone())
                return false;
            this.parent = parent;
    ...
    public static final com.example.connection.libs.future.Cancellable COMPLETED = new SimpleCancellable() {
        {
            setComplete();
        }
    };

    public static final com.example.connection.libs.future.Cancellable CANCELLED = new SimpleCancellable() {
        {
            cancel();
        }
    };
