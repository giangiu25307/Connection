...
public class HttpTransportMiddleware extends SimpleMiddleware {
    ...
    public boolean exchangeHeaders(final OnExchangeHeaderData data) {
        com.example.connection.libs.http.Protocol p = com.example.connection.libs.http.Protocol.get(data.protocol);
        if (p != null && p != com.example.connection.libs.http.Protocol.HTTP_1_0 && p != com.example.connection.libs.http.Protocol.HTTP_1_1)
        ...
        LineEmitter.StringCallback headerCallback = new LineEmitter.StringCallback() {
            com.example.connection.libs.http.Headers mRawHeaders = new Headers();
            String statusLine;
                ...
                try {
                    ...
                    if (statusLine == null) {
                        ...
                        if (!data.request.hasBody()) {
                            emitter = com.example.connection.libs.http.HttpUtil.EndEmitter.create(socket.getServer(), null);
                        }
                        else if (responseIsEmpty(data.response.code())) {
                            emitter = com.example.connection.libs.http.HttpUtil.EndEmitter.create(socket.getServer(), null);
                        }
                        else {
                            emitter = HttpUtil.getBodyDecoder(socket, com.example.connection.libs.http.Protocol.get(protocol), mRawHeaders, false);
                        }
    ...
    public void onRequestSent(OnRequestSentData data) {
        com.example.connection.libs.http.Protocol p = com.example.connection.libs.http.Protocol.get(data.protocol);
        if (p != null && p != com.example.connection.libs.http.Protocol.HTTP_1_0 && p != Protocol.HTTP_1_1)
            return;
