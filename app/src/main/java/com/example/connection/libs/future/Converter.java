...
import com.example.connection.libs.future.MultiFuture;
import com.example.connection.libs.future.MultiTransformFuture;
import com.example.connection.libs.future.SimpleFuture;
import com.example.connection.libs.future.TypeConverter;
...
public class Converter<R> {
    public static <T> Converter<T> convert(com.example.connection.libs.future.Future<T> future, String mime) {
        return new Converter<>(future, mime);
    }

    public static <T> Converter<T> convert(com.example.connection.libs.future.Future<T> future) {
    ...
    static class MultiTransformer<T, F> extends MultiTransformFuture<MimedData<com.example.connection.libs.future.Future<T>>, MimedData<com.example.connection.libs.future.Future<F>>> {
        ...
        @Override
        protected void transform(MimedData<com.example.connection.libs.future.Future<F>> converting) {
    ...
    String futureMime;
    protected Converter(com.example.connection.libs.future.Future future, String mime) {
    ...
    synchronized private final <T> com.example.connection.libs.future.Future<T> to(Object value, Class<T> clazz, String mime) {
        if (clazz.isInstance(value))
            return new SimpleFuture<>((T) value);
        return to(value.getClass(), clazz, mime);
    }

    synchronized private final <T> com.example.connection.libs.future.Future<T> to(Class fromClass, Class<T> clazz, String mime) {
        if (TextUtils.isEmpty(mime))
            mime = MIME_ALL;
        ...
        if (search(target, bestMatch, currentPath, new MimedType(fromClass, futureMime), new HashSet<MimedType>())) {
            ...
            new SimpleFuture<>(new MimedData<>((com.example.connection.libs.future.Future<Object>)future, futureMime)).setCallback(current.transformer);
    ...
    public final <T> com.example.connection.libs.future.Future<T> to(Class<T> clazz) {
