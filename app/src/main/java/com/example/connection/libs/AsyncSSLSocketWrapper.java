...
import com.example.connection.libs.AsyncSSLException;
import com.example.connection.libs.AsyncSSLServerSocket;
...
import com.example.connection.libs.http.SSLEngineSNIConfigurator;
import com.example.connection.libs.util.Allocator;
import com.example.connection.libs.util.StreamUtility;
import com.example.connection.libs.wrapper.AsyncSocketWrapper;
...
public class AsyncSSLSocketWrapper implements AsyncSocketWrapper, com.example.connection.libs.AsyncSSLSocket {
    ...
    com.example.connection.libs.AsyncSocket mSocket;
    ...
    public static void handshake(com.example.connection.libs.AsyncSocket socket,
    ...
    public static Cancellable connectSocket(com.example.connection.libs.AsyncServer server, String host, int port,InetSocketAddress local,  ConnectCallback callback) {
        return connectSocket(server, host, port, local,false, callback);
    }
    public static Cancellable connectSocket(com.example.connection.libs.AsyncServer server, String host, int port,InetSocketAddress local, boolean trustAllCerts, ConnectCallback callback) {
    ...
    private AsyncSSLSocketWrapper(com.example.connection.libs.AsyncSocket socket,
                                  String host, int port,
                                  SSLEngine sslEngine,
                                  TrustManager[] trustManagers, HostnameVerifier verifier, boolean clientMode) {
        mSocket = socket;
...
//                ex.printStackTrace();
        ...
        try {
            if (!finishedHandshake && (engine.getHandshakeStatus() == HandshakeStatus.NOT_HANDSHAKING || engine.getHandshakeStatus() == HandshakeStatus.FINISHED)) {
                if (clientMode) {
                    ...
                    if (!trusted) {
                        com.example.connection.libs.AsyncSSLException e = new AsyncSSLException(peerUnverifiedCause);
                        report(e);
                        if (!e.getIgnore())
                            throw e;
                    }
    ...
    @Override
    public com.example.connection.libs.AsyncServer getServer() {
        return mSocket.getServer();
    }

    @Override
    public com.example.connection.libs.AsyncSocket getSocket() {
        return mSocket;
    }
    ...
    public static com.example.connection.libs.AsyncSSLServerSocket listenSecure(final Context context, final com.example.connection.libs.AsyncServer server, final String subjectName, final InetAddress host, final int port, final ListenCallback handler) {
        final ObjectHolder<com.example.connection.libs.AsyncSSLServerSocket> holder = new ObjectHolder<>();
        server.run(() -> {
            try {
    ...
    public static com.example.connection.libs.AsyncSSLServerSocket listenSecure(com.example.connection.libs.AsyncServer server, String keyDer, String certDer, final InetAddress host, final int port, final ListenCallback handler) {
    ...
    public static com.example.connection.libs.AsyncSSLServerSocket listenSecure(final com.example.connection.libs.AsyncServer server, final byte[] keyDer, final byte[] certDer, final InetAddress host, final int port, final ListenCallback handler) {
        final ObjectHolder<com.example.connection.libs.AsyncSSLServerSocket> holder = new ObjectHolder<>();
        server.run(() -> {
            try {
    ...
    public static com.example.connection.libs.AsyncSSLServerSocket listenSecure(final com.example.connection.libs.AsyncServer server, final PrivateKey pk, final Certificate cert, final InetAddress host, final int port, final ListenCallback handler) {
        final ObjectHolder<com.example.connection.libs.AsyncSSLServerSocket> holder = new ObjectHolder<>();
        server.run(() -> {
            try {
                ...
                final com.example.connection.libs.AsyncServerSocket socket = listenSecure(server, sslContext, host, port, handler);
    ...
    public static com.example.connection.libs.AsyncServerSocket listenSecure(AsyncServer server, final SSLContext sslContext, final InetAddress host, final int port, final ListenCallback handler) {
