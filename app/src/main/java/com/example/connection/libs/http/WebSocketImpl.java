...
import com.example.connection.libs.http.HybiParser;
...
public class WebSocketImpl implements com.example.connection.libs.http.WebSocket {
    @Override
    public void end() {
        mSocket.end();
    }
    ...
    private void setupParser(boolean masking, boolean deflate) {
        mParser = new com.example.connection.libs.http.HybiParser(mSocket) {
            @Override
...
//            response.getHeaders().getHeaders().set("Access-Control-Allow-Origin", "http://" + origin);
    ...
    public static void addWebSocketUpgradeHeaders(AsyncHttpRequest req, String... protocols) {
        com.example.connection.libs.http.Headers headers = req.getHeaders();
...
//                return null;
    ...
    com.example.connection.libs.http.HybiParser mParser;
