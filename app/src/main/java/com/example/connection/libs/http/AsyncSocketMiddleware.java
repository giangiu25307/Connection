...
import com.example.connection.libs.future.Futures;
...
import com.example.connection.libs.util.ArrayDeque;
...
public class AsyncSocketMiddleware extends SimpleMiddleware {
    ...
    public AsyncSocketMiddleware(com.example.connection.libs.http.AsyncHttpClient client, String scheme, int port) {
        mClient = client;
        this.scheme = scheme;
        this.port = port;
    }
    ...
    public AsyncSocketMiddleware(com.example.connection.libs.http.AsyncHttpClient client) {
        this(client, "http", 80);
    }
    ...
    private void recycleSocket(final AsyncSocket socket, com.example.connection.libs.http.AsyncHttpRequest request) {
        if (socket == null)
            return;
    ...
    protected boolean isKeepAlive(OnResponseCompleteData data) {
        return com.example.connection.libs.http.HttpUtil.isKeepAlive(data.response.protocol(), data.response.headers()) && HttpUtil.isKeepAlive(Protocol.HTTP_1_1, data.request.getHeaders());
    }
